#!/usr/bin/env bash
# nmap — Nmap reference tool. Use when working with nmap in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
nmap v$VERSION — Nmap Reference Tool
...
## What is Nmap?
Nmap (nmap) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with nmap.

## Key Concepts
- Core nmap principles and fundamentals
- How nmap fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Nmap Matters
Understanding nmap is critical for:
...
## Getting Started
1. Understand the basic nmap concepts
...
## Installation
1. Download or clone the nmap package
...
cmd_patterns() {
    cat << 'EOF'
# Nmap — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for nmap
...
case "$CMD" in
    ...
    version|--version|-v) echo "nmap v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: nmap help"; exit 1 ;;
esac
