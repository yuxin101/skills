#!/usr/bin/env bash
# sortino — Sortino reference tool. Use when working with sortino in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
sortino v$VERSION — Sortino Reference Tool

Usage: sortino <command>
...
## What is Sortino?
Sortino (sortino) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with sortino.

## Key Concepts
- Core sortino principles and fundamentals
- How sortino fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Sortino Matters
Understanding sortino is critical for:
...
## Getting Started
1. Understand the basic sortino concepts
...
cmd_formulas() {
    cat << 'EOF'
# Sortino — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Sortino — Instruments & Tools Overview

## Primary Instruments
- Core tools used in sortino operations
...
cmd_glossary() {
    cat << 'EOF'
# Sortino — Key Terms & Definitions

## Core Terminology
- **Sortino**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "sortino v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: sortino help"; exit 1 ;;
esac
