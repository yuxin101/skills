...
TRADE_SOURCE = "sdk:polymarket-48h-weather-distribution-trader"
SKILL_SLUG   = "polymarket-48h-weather-distribution-trader"
...
def run(live: bool = False) -> None:
    ...
    print(f"[48h-weather-dist] mode={mode} max_pos=${MAX_POSITION} sum_tol={SUM_TOLERANCE:.0%}")

    client = get_client(live=live)
    markets = find_markets(client)
    print(f"[48h-weather-dist] {len(markets)} candidate markets")

    # Build temperature distributions
    distributions = build_distributions(markets)
    print(f"[48h-weather-dist] {len(distributions)} distributions: "
    ...
    print(f"[48h-weather-dist] {len(all_opps)} violation opportunities")
    ...
    print(f"[48h-weather-dist] done. {placed} orders placed.")
