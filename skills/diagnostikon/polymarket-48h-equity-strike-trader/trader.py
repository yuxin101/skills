...
TRADE_SOURCE = "sdk:polymarket-48h-equity-strike-trader"
SKILL_SLUG   = "polymarket-48h-equity-strike-trader"
...
def run(live: bool = False) -> None:
    ...
    print(f"[48h-equity-strike] mode={mode} max_pos=${MAX_POSITION} min_violation={MIN_VIOLATION:.0%}")

    client = get_client(live=live)
    markets = find_markets(client)
    print(f"[48h-equity-strike] {len(markets)} candidate markets")

    # Build implied strike curves
    curves = build_curves(markets)
    print(f"[48h-equity-strike] {len(curves)} curves: {', '.join(f'{k}({len(v)} pts)' for k, v in curves.items())}")
    ...
    print(f"[48h-equity-strike] {len(all_opps)} violation opportunities")
    ...
    print(f"[48h-equity-strike] done. {placed} orders placed.")
