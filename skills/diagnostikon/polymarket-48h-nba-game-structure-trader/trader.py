...
TRADE_SOURCE = "sdk:polymarket-48h-nba-game-structure-trader"
SKILL_SLUG   = "polymarket-48h-nba-game-structure-trader"
...
def run(live: bool = False) -> None:
    ...
    print(f"[48h-nba-game-structure] mode={mode} max_pos=${MAX_POSITION} "
          f"min_inconsistency={MIN_INCONSISTENCY:.0%}")

    client = get_client(live=live)
    markets = find_markets(client)
    print(f"[48h-nba-game-structure] {len(markets)} candidate markets")

    # Group markets by game
    groups = build_game_groups(markets)
    print(f"[48h-nba-game-structure] {len(groups)} games found: "
    ...
    print(f"[48h-nba-game-structure] {len(all_opps)} inconsistency opportunities")
    ...
    print(f"[48h-nba-game-structure] done. {placed} orders placed.")
