...
TRADE_SOURCE = "sdk:polymarket-48h-player-prop-consistency-trader"
SKILL_SLUG   = "polymarket-48h-player-prop-consistency-trader"
...
def run(live: bool = False) -> None:
    ...
    print(f"[48h-player-prop-consistency] mode={mode} max_pos=${MAX_POSITION} "
          f"min_divergence={MIN_DIVERGENCE:.0%}")

    client = get_client(live=live)
    markets = find_markets(client)
    print(f"[48h-player-prop-consistency] {len(markets)} candidate markets")

    # Group markets by player
    players = group_by_player(markets)
    print(f"[48h-player-prop-consistency] {len(players)} players found: "
    ...
    print(f"[48h-player-prop-consistency] {len(best_opps)} trade opportunities")
    ...
    print(f"[48h-player-prop-consistency] done. {placed} orders placed.")
