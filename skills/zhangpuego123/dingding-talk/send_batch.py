...
def main():
    ...
    if sys.argv[1] == "--next":
        ...
        if test_mode:
            ...
            print(f"[测试模式] 第 1 条执行 Step 1-10，后续执行 Step 3-10\n")
    ...
    if total_contacts == 1:
        # 只有一个人：执行完整流程 (Step 1-10)
        print("[模式] 单人发送 - 执行完整流程 (Step 1-10)")
        success, err = send_message_to_contact(current_contact, message)
    elif is_first:
        # 多个人中的第一个：执行 Step 1-9（不还原窗口）
        print("[模式] 多人发送 - 第一个人 - 执行 Step 1-9")
        success, err = send_message_skip_steps(current_contact, message, skip_first=False, skip_last=True)
    elif is_last:
        # 多个人中的最后一个：执行 Step 3-10（跳过打开钉钉和最大化，执行还原窗口）
        print("[模式] 多人发送 - 最后一个人 - 执行 Step 3-10")
        success, err = send_message_skip_steps(current_contact, message, skip_first=True, skip_last=False)
    else:
        # 多个人中的中间：执行 Step 3-9（跳过打开钉钉、最大化和还原窗口）
        print("[模式] 多人发送 - 中间 - 执行 Step 3-9")
        success, err = send_message_skip_steps(current_contact, message, skip_first=True, skip_last=True)
