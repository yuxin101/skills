# Admin REST API (curated reference)

This hand-maintained page consolidates the **Shopify Admin REST API** `curl` reference from the monorepo folder `shopify-admin-api-1.0.0/` (bundle originally titled *Shopify Admin API* on ClawHub). It is **not** a substitute for current [shopify.dev](https://shopify.dev/) — confirm **API version**, resource paths, and rate limits there before production use.

**Snapshot:** Example URLs use Admin API version **`2024-10`**. Replace with a [supported API version](https://shopify.dev/docs/api/usage/versioning) for your app.

For **GraphQL** Admin API (`graphql.json`), see [DOC_ADMIN_API.md](DOC_ADMIN_API.md).
