...
def phase_script(name: str) -> Path:
    local = SCRIPT_DIR / name
    if local.exists():
        return local
    fallback = LEGACY_SCRIPT_DIR / name
    return fallback
...
def main() -> None:
    ...
    ap.add_argument("--scoring-v2", action="store_true", help="Enable Scoring V2 path (rule filter + scoring)")
    args = ap.parse_args()
    ...
    manifest: Dict[str, Any] = {
        ...
        "scoring_v2": bool(args.scoring_v2),
    }
    ...
    try:
        ...
        if not args.skip_core:
            code = run_cmd(
                [
                    args.python,
                    str(phase_script("core_pipeline.py")),
                    "--sources",
                    str(sources),
                    "--dedup",
                    str(staged_dedup),
        ...
        if not should_skip_generation:
            phase_a_input = staged_new
            if args.scoring_v2:
                rule_cmd = [
                    args.python,
                    str(phase_script("phase_rule_filter_score.py")),
                    "--input",
                    str(staged_new),
                    "--output",
                ...
                phase_a_input = staged_rule_filtered

            scoring_script = "phase_model_score.py" if args.scoring_v2 else "phase_a_score.py"
            scoring_output = staged_model_scored if args.scoring_v2 else staged_scored
            phase_a_cmd = [
                args.python,
                str(phase_script(scoring_script)),
                "--input",
                str(phase_a_input),
                "--output",
                str(scoring_output),
            ...
            phase_b_input = staged_scored
            if args.scoring_v2:
                rank_cmd = [
                    args.python,
                    str(phase_script("phase_rank_distribute.py")),
                    "--input",
            ...
            phase_b_cmd = [
                args.python,
                str(phase_script("phase_b_analyze.py")),
                "--input",
                str(phase_b_input),
                "--output",
                str(staged_deep),
            ...
            if args.scoring_v2:
                phase_b_cmd.append("--preselected")
            if args.mock:
            ...
            digest_cmd = [
                args.python,
                str(phase_script("digest_writer.py")),
                "--scored",
                str(staged_scored),
                "--deep-set",
                str(staged_deep),
            ...
            if args.scoring_v2:
                digest_cmd += ["--other-set", str(staged_other)]
            ...
            render_cmd = [
                args.python,
                str(phase_script("render_digest_pdf_nextdraft.py")),
                "--input",
                str(staged_digest),
                "--html-output",
                str(staged_html),
