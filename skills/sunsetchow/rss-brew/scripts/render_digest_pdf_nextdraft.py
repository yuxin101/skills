    ...
    for raw in lines:
        ...
        if line.startswith('### '):
            ...
            current = {
                ...
                'en_summary': '', 'zh_summary': '', 'deep_analysis': [], 'underwater': '', 'golden_quotes': [], 'further_reading': [], 'note': ''
            }
            last_field = None
            continue
        ...
        for prefix, key in field_prefixes:
            if candidate.startswith(prefix):
                ...
                current[key] = [] if key in ('deep_analysis', 'further_reading') else val
                last_field = key
                matched = True
                break
        if matched:
        ...
        if line.startswith('  - '):
            ...
            elif last_field == 'further_reading':
                # Parse markdown link: [title](url)
                m = re.match(r'^\[(.*?)\]\((https?://[^\s)]+)\)\s*$', item)
                if m:
                    current['further_reading'].append({'title': m.group(1), 'url': m.group(2)})
            elif last_field:
                if isinstance(current.get(last_field), list):
                    continue
        ...
        if line.strip() and last_field:
            if last_field == 'deep_analysis':
                ...
                # Ignore malformed continuation lines under Further Reading.
                pass
            else:
                if isinstance(current.get(last_field), list):
                    continue
...
def render_html(data: Dict[str, Any]) -> str:
    ...
    for i, a in enumerate(articles, 1):
        ...
        <article class="entry">
          ...
          {f'<section class="further-reading"><h3>Further Reading</h3><ul>' + ''.join(f'<li><a href="{_safe_href(w.get("url", ""))}">{esc(w.get("title", ""))}</a></li>' for w in a.get("further_reading", []) if _safe_href(w.get("url", ""))) + '</ul></section>' if a.get('further_reading') else ''}
          {f'<div class="source-link"><a href="{_safe_href(a["url"])}">Source</a></div>' if _safe_href(a['url']) else ''}
        </article>
        ''')
    ...
    for a in others:
        ...
        <div class="skim-item">
          ...
          {paras(summary)}
          {f'<div class="source-link small"><a href="{_safe_href(a["url"])}">Source</a></div>' if _safe_href(a['url']) else ''}
        </div>
        ''')
...
.further-reading {{ margin-top: 14px; }}
.further-reading h3 {{ font-size: 9.5pt; text-transform: uppercase; letter-spacing: .06em; color: #555; margin-bottom: 6px; }}
.further-reading ul {{ margin: 0; padding-left: 16px; }}
.further-reading li {{ font-size: 9pt; line-height: 1.5; margin-bottom: 3px; }}
.further-reading a {{ color: #1a5f9e; text-decoration: none; }}
