...
def _extract_json_object(content: str) -> Dict[str, Any]:
    """Extract first valid JSON object from LLM output, handling common GLM quirks.

    Handles:
    - Leading/trailing whitespace or markdown
    - Trailing explanation text after closing brace ("Extra data" error)
    - Multiple JSON objects (only first is used)
    """
    try:
    ...
    # Find first '{' and walk the string to find its matching '}'
    start = content.find("{")
    if start < 0:
    ...
    raise json.JSONDecodeError("Unterminated JSON object", content, len(content))


def _build_web_context_block(article: Dict[str, Any]) -> str:
    enrichment = article.get("enrichment") if isinstance(article.get("enrichment"), dict) else {}
    web_context = enrichment.get("web_context") if isinstance(enrichment, dict) else None
    if not isinstance(web_context, list) or not web_context:
        return ""

    lines = ["", "Web Context (supporting references, use as background only):"]
...
def analyze_one(article: Dict[str, Any], model_alias: str, mock: bool = False) -> Dict[str, Any]:
    ...
    prompt += _build_web_context_block(article)
    if mock:
        data = {
            "category": "other",
        ...
        for attempt in range(3):
            try:
                ...
                last_exc = exc
                if attempt < 2:
                    print(f"[phase_b] retry {attempt + 1}/2 for '{article.get('title', '')[:50]}': {exc}", file=sys.stderr)
        else:
            raise last_exc
...
def main() -> None:
    ...
    for i, article in enumerate(deep_set, start=1):
        try:
            ...
            print(f"[phase_b] SKIP index={i-1} (all retries failed): {e}", file=sys.stderr)
            # Degrade gracefully: include article with basic info, no deep analysis
            fallback = dict(article)
            fallback.setdefault("category", "other")
            fallback.setdefault("english_summary", "(Analysis unavailable)")
            fallback.setdefault("chinese_summary", "(分析不可用)")
