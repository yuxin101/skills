...
def get_deepset_ineligible_sources(sources: List[Source]) -> Set[str]:
    """Return a set of source names that are ineligible for the deep set."""
    return {s.name for s in sources if not s.deepset_eligible}
...
def _ranked(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for a in items:
        ...
        # Rule score (int, max 5ish) + Model score (float 0-5) = Final Score (float 0-10ish)
...
def _select_deep(
    ...
    deepset_ineligible: Optional[Set[str]] = None,
) -> Tuple[List[Dict[str, Any]], Dict[str, int]]:
    if not ranked or target <= 0:
        return [], {"blocked_low_conf_top3": 0, "blocked_source_cap": 0, "blocked_topic_cap": 0, "blocked_deepset_ineligible": 0}

    allowed_deep = min(target, max(1, len(ranked) - max(0, min_other)))
    selected: List[Dict[str, Any]] = []
    source_used: Dict[str, int] = {}
    topic_used: Dict[str, int] = {}
    blocked = {"blocked_low_conf_top3": 0, "blocked_source_cap": 0, "blocked_topic_cap": 0, "blocked_deepset_ineligible": 0}

    if deepset_ineligible is None:
        deepset_ineligible = set()
    ...
    for item in ranked:
        ...
        source_lower = source.lower() or "unknown"
        topic = _normalized_topic(item)

        # Skip if source is ineligible for deep set
        if source in deepset_ineligible:
            blocked["blocked_deepset_ineligible"] += 1
            continue

        if source_used.get(source_lower, 0) >= max(1, source_cap):
        ...
        source_used[source_lower] = source_used.get(source_lower, 0) + 1
...
def main() -> None:
    ...
    ap.add_argument("--sources", default=None, help="Path to sources.yaml for deepset eligibility filtering")
    ...
    # Load sources for deepset eligibility filtering
    deepset_ineligible: Optional[Set[str]] = None
    if args.sources:
        sources = load_sources(Path(args.sources))
        deepset_ineligible = get_deepset_ineligible_sources(sources)
    ...
    deep, blocked = _select_deep(
        ...
        deepset_ineligible=deepset_ineligible,
    )
