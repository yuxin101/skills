# Contributing to founderclaw

Thanks for wanting to make founderclaw better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

founderclaw skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `founderclaw/` (your global install). But when you're developing founderclaw itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd founderclaw
...
Contributor mode turns founderclaw into a self-improving tool. Enable it and Claude Code
will periodically reflect on its founderclaw experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `founderclaw/data/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
founderclaw/bin/founderclaw-config set founderclaw_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork founderclaw, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use founderclaw normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls founderclaw/data/contributor-logs/`
3. **Fork and clone founderclaw** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where founderclaw annoyed you)
   ln -sfn /path/to/your/founderclaw-fork .claude/skills/founderclaw
   cd .claude/skills/founderclaw && bun install && bun run build && ./setup
   ```
   Setup creates the per-skill symlinks (`qa -> founderclaw/qa`, etc.) and asks your
   prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using founderclaw** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix founderclaw while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ founderclaw sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on founderclaw inside the founderclaw repo

When you're editing founderclaw skills and want to test them by actually using founderclaw
...
```
founderclaw/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── founderclaw -> ../../         <- symlink back to repo root
│   ├── review -> founderclaw/review  <- short names (default)
│   ├── ship -> founderclaw/ship      <- or founderclaw-review, founderclaw-ship if --prefix
...
Skill symlink names depend on your prefix setting (`founderclaw/data/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/founderclaw-review`, `/founderclaw-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `~/.founderclaw-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.founderclaw-dev/` for post-mortem debugging and trend analysis.
...
founderclaw generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/founderclaw-{skill}/SKILL.md` (generated at setup, gitignored) |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/founderclaw` | `$FOUNDERCLAW_ROOT` (`.agents/skills/founderclaw` in a repo, otherwise `~/.codex/skills/founderclaw`) |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/founderclaw`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop founderclaw.** Symlink your founderclaw checkout
...
```bash
# In your core project (not the founderclaw repo)
ln -sfn /path/to/your/founderclaw-checkout .claude/skills/founderclaw
```

### Step 2: Run setup to create per-skill symlinks

The `founderclaw` symlink alone isn't enough. Claude Code discovers skills through
individual symlinks (`qa -> founderclaw/qa`, `ship -> founderclaw/ship`, etc.), not through
the `founderclaw/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/founderclaw && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/founderclaw-qa`).
Your choice is saved to `founderclaw/data/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `founderclaw/`:

```bash
rm .claude/skills/founderclaw
```

The per-skill symlinks (`qa`, `ship`, etc.) still point to `founderclaw/...`, so they'll
resolve to the global install automatically.

### Switching prefix mode

If you vendored founderclaw with one prefix setting and want to switch:

```bash
cd .claude/skills/founderclaw && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/founderclaw && ./setup --prefix       # switch to /founderclaw-qa, /founderclaw-ship
```
