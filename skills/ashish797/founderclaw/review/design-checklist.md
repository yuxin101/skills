...
**Trigger:** Only run this checklist if the diff touches frontend files. Use `founderclaw-diff-scope` to detect:

```bash
source <(~/.claude/skills/founderclaw/bin/founderclaw-diff-scope <base> 2>/dev/null)
```
