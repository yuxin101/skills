...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/founderclaw/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.founderclaw/projects/$REMOTE_SLUG/greptile-history.md"
```
...
Use Tier 2 when escalation detection (below) identifies a prior FounderClaw reply on the same thread. Include maximum evidence to close the discussion.
...
Before composing a reply, check if a prior FounderClaw reply already exists on this comment thread:

1. **For line-level comments:** Fetch replies via `gh api repos/$REPO/pulls/$PR_NUMBER/comments/$COMMENT_ID/replies`. Check if any reply body contains FounderClaw markers: `**Fixed**`, `**Not a bug.**`, `**Already fixed**`.

2. **For top-level comments:** Scan the fetched issue comments for replies posted after the Greptile comment that contain FounderClaw markers.

3. **If a prior FounderClaw reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior FounderClaw reply exists:** Use Tier 1 (friendly) templates.
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/founderclaw/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.founderclaw/projects/$REMOTE_SLUG"
mkdir -p ~/.founderclaw
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.founderclaw/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.founderclaw/greptile-history.md` (global aggregate)
...
Example entries:
```
2026-03-13 | ashish797/myapp | fp | app/services/auth_service.rb | race-condition
2026-03-13 | ashish797/myapp | fix | app/models/user.rb | null-check
2026-03-13 | ashish797/myapp | already-fixed | lib/payments.rb | error-handling
```
