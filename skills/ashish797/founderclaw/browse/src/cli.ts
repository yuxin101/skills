/**
 * founderclaw CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .founderclaw/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and founderclaw
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`founderclaw browse — Fast headless browser for AI coding agents
  ...
  if (command === 'connect') {
    ...
    try {
      ...
      try {
        ...
        const agentQueue = path.join(process.env.HOME || '/tmp', '.founderclaw', 'sidebar-agent-queue.jsonl');
