/**
 * founderclaw browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.founderclaw/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.founderclaw/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['founderclaw browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.founderclaw', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'founderclaw', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'founderclaw', 'browse', 'dist', 'browse'),
  ];
...
function shortenPath(str: string): string {
  return str
    ...
    .replace(/\.claude\/skills\/founderclaw\//g, '')
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.founderclaw', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.founderclaw', 'sidebar-agent-queue.jsonl');
  const founderclawDir = path.dirname(agentQueue);
...
async function shutdown() {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.founderclaw', 'chromium-profile');
