...
Analyzes US stocks and top 20 cryptocurrencies using public market data APIs.

This is the MINIMAL BUNDLE version - includes core analysis features only.
Advanced features (SEC EDGAR, Fear & Greed Index) require additional dependencies.
...
Data Sources (Minimal Bundle):
------------------------------
- Yahoo Finance: Prices, fundamentals, earnings, analyst ratings
- CoinGecko: Crypto market data, trending rankings
- Google News: Financial news headlines (read-only RSS)

All requests are read-only HTTP GET. No data upload or POST requests.

Usage:
-----
    # Single stock analysis
    python3 stock_analyzer.py AAPL

    # Multiple stocks
    python3 stock_analyzer.py AAPL MSFT GOOGL

    # Fast mode (skip slow analyses)
    python3 stock_analyzer.py AAPL --fast

    # Crypto analysis
    python3 stock_analyzer.py BTC-USD ETH-USD

    # JSON output
    python3 stock_analyzer.py AAPL --output json

Performance (Minimal Bundle):
----------------------------
- Default mode: 45-75s (full analysis)
- --fast mode: 30-45s (skip news sentiment)
...
--------
    ...
    >>> python3 stock_analyzer.py AAPL --verbose

    >>> # Compare tech stocks
    >>> python3 stock_analyzer.py AAPL MSFT GOOGL NVDA

    >>> # Quick crypto check
    >>> python3 stock_analyzer.py BTC-USD --fast
...
def check_breaking_news(verbose: bool = False) -> list[str] | None:
    """
    Breaking news monitoring (not available in minimal bundle).

    This feature requires feedparser package which is not included in the
    minimal bundle. Returns None.

    For breaking news analysis, use the full version from GitHub:
...
async def get_fear_greed_index() -> tuple[float, int | None, str | None] | None:
    """
    Fear & Greed Index (not available in minimal bundle).

    This feature requires fear-and-greed package which is not included in the
    minimal bundle. Returns None.

    For Fear & Greed Index analysis, use the full version from GitHub:
    https://github.com/ZhenRobotics/openclaw-research-analyst
    """
    return None
...
async def get_insider_activity(ticker: str, period_days: int = 90) -> tuple[float, int | None, float | None] | None:
    """
    Insider trading analysis (not available in minimal bundle).

    This feature requires edgartools package which is not included in the
    minimal bundle to keep dependencies lightweight. Returns None.

    For insider trading analysis, use the full version from GitHub:
    https://github.com/ZhenRobotics/openclaw-research-analyst
    """
    return None
