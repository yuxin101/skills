...
# 克隆或下载项目到本地
# 示例: 你的项目目录，根据实际情况调整
git clone <repository-url> xhs-mcp-service
cd xhs-mcp-service
...
await publishContent({
  ...
  images: ['./photo1.jpg', './photo2.jpg'],  // 使用相对或绝对路径
  tags: ['美食', '探店'],
  visibility: '公开可见'
});
```
...
const result = await publishContent({
  ...
  images: [
    './photo1.jpg',           // 相对路径（相对于项目根目录）
    '/absolute/path/photo2.jpg',  // 或绝对路径
...
# 生产环境（使用PM2）
npm install -g pm2
pm2 start src/index.js --name xhs-mcp
...
# 使用cron定时发布
# 每天上午10点发布
0 10 * * * cd /你的项目路径/xhs-mcp-service && node publish-daily.js
```

```javascript
// publish-daily.js（在项目根目录）
import { publishContent } from './src/xhs-tools.js';

await publishContent({
  title: '早安打卡',
  content: '今天的早安打卡...',
  images: ['./daily-photo.jpg'],  // 相对路径
  tags: ['早安', '打卡'],
  visibility: '公开可见'
});
```
...
$env:XHS_PORT=18061          # Windows PowerShell
set XHS_PORT=18061           # Windows CMD

# 绑定地址（默认0.0.0.0）
export XHS_HOST=0.0.0.0      # Linux/Mac
$env:XHS_HOST="0.0.0.0"      # Windows

# 代理设置（可选）
export XHS_PROXY=http://proxy:port     # Linux/Mac
$env:XHS_PROXY="http://proxy:port"     # Windows
```
