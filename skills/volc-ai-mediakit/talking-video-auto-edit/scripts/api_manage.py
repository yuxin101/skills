...
class ApiManage:
    ...
    # ----------------------------
    def _get_vod_service(self) -> VodService:
        vod = VodService(region=os.getenv("VOLC_REGION", "cn-north-1"))
        ak = os.getenv("VOLC_ACCESS_KEY_ID", "").strip()
        sk = os.getenv("VOLC_ACCESS_KEY_SECRET", "").strip()
        if not ak or not sk:
            raise ValueError("VOLC_ACCESS_KEY_ID / VOLC_ACCESS_KEY_SECRET must be set in environment")
        vod.set_ak(ak)
        vod.set_sk(sk)
        host = os.getenv("VOLC_HOST", "").strip()
        if host:
            vod.set_host(host)
        return vod
    ...
    def upload_media_auto(self, source: str, space_name: Optional[str] = None, file_ext: str = ".mp4") -> Dict[str, Any]:
        ...
        - 否则：本地文件上传（VodService.upload_media），返回 type=local + Vid/DirectUrl 等
        """
        ...
        vod = self._get_vod_service()
        req = VodUploadMediaRequest()
        req.SpaceName = space_name
        req.FilePath = str(p)
        # 使用 UUID 作为 FileName，避免原文件名含特殊字符/中文等导致异常
        req.FileName = f"{uuid.uuid4().hex}{file_ext}"
        req.FileExtension = file_ext
        req.StorageClass = 1

        resp = vod.upload_media(req)
        ...
        return {
            ...
            "DirectUrl": data.SourceInfo.FileName,
        }
