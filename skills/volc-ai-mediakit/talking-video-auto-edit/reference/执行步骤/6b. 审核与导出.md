...
>
> **前置要求**：必须先 `cd ./scripts`；默认是否自动打开浏览器由 `TALKING_VIDEO_AUTO_EDIT_REVIEW_AUTO_OPEN` 决定，可加 `--no-open` 覆盖

# 检查单

- [ ] **示例参考**：`examples/step6_speech_cut.json`
- [ ] **启动审核页面前置判断**：默认不自动打开；是否打开由 `TALKING_VIDEO_AUTO_EDIT_REVIEW_AUTO_OPEN` 决定（`1`/`true`/`yes` 时打开）。
- [ ] **启动审核页服务**：`uv run ./serve_review_page.py [--output-dir output/<文件名>]`（默认 5173，端口冲突时自动使用其他端口，以启动时打印的 URL 为准）
- [ ] **启动导出服务**：`uv run ./export_server.py [--output-dir output/<文件名>]`（默认 http://127.0.0.1:7860/export）
...
```mermaid
flowchart TD
    Start([开始 Step6b]) --> S1[uv run serve_review_page.py]
    S1 --> S2[uv run export_server.py]
