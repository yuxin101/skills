...
>
> **前置要求**：必须先 `cd <SKILL_DIR>/scripts` 再执行本步骤命令（避免 uv 环境不一致）
>
...
- [ ] Run：
  - 必须先进入 scripts 目录（避免 uv 环境不一致）：
    - `cd ./scripts`
  - **URL 或本地文件（会执行上传）**：`uv run ./pipeline_url_to_asr.py "<url_or_local_path>" --ext ".mp4" [--output-dir output/<文件名>]`
  - **已有 Vid（跳过上传）**：`uv run ./pipeline_url_to_asr.py --vid "<vid>" --ext ".mp4" [--output-dir output/<文件名>]`
  - **已有 DirectUrl（VOD 空间内 FileName，跳过上传）**：`uv run ./pipeline_url_to_asr.py --directurl "<filename>" --ext ".mp4" [--output-dir output/<文件名>]`
- [ ] **用户输入说明**：
...
flowchart TD
    ...
    Cd --> Run[uv run pipeline_url_to_asr.py<br/>+ 对应参数与 --output-dir]
