...
>
> **前置要求**：必须先 `cd ./scripts` 再执行命令

# 检查单

- [ ] **重复文件检查**：若 `output/<文件名>/export_request.json` 或 `review_import_data.json` 已存在，**必须提示用户**：「导出数据已存在，是否删除/覆盖/保留并写入新目录(01)？」用户确认后再执行
- [ ] Run（需先 `cd ./scripts`）：`uv run ./prepare_export_data.py --write-step6 [--output-dir output/<文件名>]`（画布与视频源从 step1 读取，可加 `--width` `--height` 指定）
- [ ] **脚本职责**：
...
flowchart TD
    ...
    Ask --> Start
    Dup -->|否| Cd[cd ./scripts]
    Cd --> Run[uv run prepare_export_data.py<br/>--write-step6 + --output-dir]
