...
>
> **参考文档**：`reference/用户规则/*.md`、`reference/内置/EDL编辑决策格式.md`、`examples/edl.json`

# 检查单

- [ ] **重复文件检查**：若 `output/<文件名>/step6_speech_cut.json` 已存在，**必须提示用户**：「step6_speech_cut.json 已存在，是否删除/覆盖/保留并写入新目录(01)？」用户确认后再执行
...
- [ ] **内容处理**（先执行逻辑，再按 EDL 格式输出）
  - [ ] 参考 `reference/用户规则/README.md` 的优先级顺序，生成「建议删除/保留」的剪辑方案
  - [ ] **语义重复检查（必做）**：参考 `reference/用户规则/4-重复句检测.md`，**先**逐对检查相邻句语义重复。典型：前句「XX 原理是/其实是」+ 后句「我来说一下 XX 流程/方法」→ 前句为过渡铺垫，**整段 delete**，不做句内 concat。
...
- [ ] **EDL 输出格式**（见 `reference/内置/EDL编辑决策格式.md`、`examples/edl.json`）
...
flowchart TD
    ...
    Ask --> Start
    Dup -->|否| Read[读 step5 优化结果 / raw / 规则 README]
