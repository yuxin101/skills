---
name: talking-video-auto-edit
display_name: 口播视频智能精剪工具
version: 1.0.7
description: |
  火山引擎 AI MediaKit 口播剪辑 Skill， 剪口播一站式技能。涵盖环境准备、媒资管理、音频处理、口播剪辑、视频导出、审阅页面、自迭代机制全流程能力。当用户提及"口播剪辑"、"剪口播"、"剪视频"、"去掉停顿"、"处理音频"、"导出口播"、"自动剪辑"、"去除口误"、"视频剪辑"时，必须触发此 Skill。上传视频/音频文件要求剪辑时也须触发
category: 音视频处理
env:
...
> - **本地 `.env`**：技能从 `<SKILL_DIR>/.env` 读取配置（`setup.sh` 可在缺失时从 `template/env.md` **生成空壳文件**）。**密钥须由用户在本地手工填入**；Agent **不得**在对话或仓库中写入真实 Secret，亦不得将 `.env` 提交到版本库。请保持 `.gitignore` 忽略 `.env`（若仓库尚无则在本技能目录补充）。
...
| Step     | 说明                            | 文档                                                                                  |
| -------- | ------------------------------- | ------------------------------------------------------------------------------------- |
| Step 1   | 环境检查与依赖安装              | [1. 环境检查.md](reference/执行步骤/1.%20环境检查.md)                                 |
| Step 2   | 语气词 / 卡顿词确认与规则更新   | [2. 语气词提示与用户行为更新.md](reference/执行步骤/2.%20语气词提示与用户行为更新.md) |
| Step 3   | URL → ASR 流水线与候选生成      | [3. URL到ASR流水线与候选生成.md](reference/执行步骤/3.%20URL到ASR流水线与候选生成.md) |
| Step 4   | 宿主 Agent ASR 语义纠错（必须） | [4. ASR语义纠错.md](reference/执行步骤/4.%20ASR语义纠错.md)                           |
| Step 5   | 宿主 Agent 口播剪辑             | [5. 口播剪辑.md](reference/执行步骤/5.%20口播剪辑.md)                                 |
| Step 5.5 | 审核逻辑确认（是否打开审核页）  | [5.5 审核逻辑确认.md](reference/执行步骤/5.5%20审核逻辑确认.md)                       |
| Step 6a  | 数据预处理                      | [6a. 数据预处理.md](reference/执行步骤/6a.%20数据预处理.md)                           |
| Step 6b  | 审核与导出                      | [6b. 审核与导出.md](reference/执行步骤/6b.%20审核与导出.md)                           |
| Step 6c  | VOD 导出任务提交与查询          | [6c. VOD导出任务提交与查询.md](reference/执行步骤/6c.%20VOD导出任务提交与查询.md)     |
