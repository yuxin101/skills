#!/usr/bin/env bash
# tailwind — Tailwind reference tool. Use when working with tailwind in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
tailwind v$VERSION — Tailwind Reference Tool

Usage: tailwind <command>
...
## What is Tailwind?
Tailwind (tailwind) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with tailwind.

## Key Concepts
- Core tailwind principles and fundamentals
- How tailwind fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Tailwind Matters
Understanding tailwind is critical for:
...
## Getting Started
1. Understand the basic tailwind concepts
...
## Installation
1. Download or clone the tailwind package
...
cmd_patterns() {
    cat << 'EOF'
# Tailwind — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for tailwind
...
cmd_security() {
    cat << 'EOF'
# Tailwind — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Tailwind — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "tailwind v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tailwind help"; exit 1 ;;
esac
