...
INDEX_NAMES = {
    ...
    '000001': '上证指数', '000016': '上证50', '000688': '科创50', '000852': '中证1000',
}
...
def get_daily_kline(code, days=90):
    """获取日K数据 - 优先使用akshare"""
    index_codes_ak = {
        '399001': 'sz399001', '399006': 'sz399006', '399300': 'sh399300',
        '000001': 'sh000001', '000016': 'sh000016', '000688': 'sh000688', '000852': 'sh000852',
    }
    ...
    try:
        ...
        # 指数优先使用akshare
        if code in index_codes_ak:
            try:
                symbol = index_codes_ak[code]
                ...
                if df is not None and len(df) > 0:
                    ...
                    for _, row in df.iterrows():
                        try:
                            klines.append({'date': str(row['date']), 'open': float(row['open']), 
                ...
                print(f"akshare指数日K失败: {e}", file=sys.stderr)
...
def get_minute_kline(code, period='30'):
    ...
    try:
        if is_index:
            # 指数使用东方财富接口
            try:
                df = ak.index_zh_a_hist_min_em(symbol=code, period=freq)
                if df is not None and len(df) > 0:
                    df = df.tail(500)
                    ...
                    for _, row in df.iterrows():
                        try:
                            klines.append({'datetime': str(row['时间']), 'open': float(row['开盘']), 'high': float(row['最高']),
                                           'low': float(row['最低']), 'close': float(row['收盘']), 'volume': float(row['成交量'])})
                        except:
                            continue
                    return klines
            except Exception as e:
                print(f"akshare指数分钟失败: {e}", file=sys.stderr)
        else:
            # 股票使用akshare
            try:
                ...
                print(f"akshare分钟K失败: {e}", file=sys.stderr)
    except Exception as e:
        print(f"分钟K获取失败: {e}", file=sys.stderr)
...
def generate_report(stock_name, stock_code, daily, m30, m5, m1):
    ...
    def level_report(analysis, name):
        if not analysis:
            return [f"{name}：数据不足"]
        m = analysis
        l = m['levels']
        macd = m['macd'] or {}
        lines = [f"{name}：", f"  当前价格：{l['current']:.2f}"]
        if name == '日线':
            ma_info = [f"MA{p}={l[f'ma{p}']:.2f}" for p in [5, 13, 21, 55, 89, 144] if f'ma{p}' in l]
            lines.append(f"  均线：{' | '.join(ma_info)}")
            lines.append(f"  MA89：{'上方' if l.get('ma89_pos') == 'above' else '下方'}")
            lines.append(f"  MA144：{'上方' if l.get('ma144_pos') == 'above' else '下方'}")
        if macd:
            lines.append(f"  MACD：DIF={macd['dif']:.3f}，DEA={macd['dea']:.3f}，柱={macd['macd']:.3f}，{'多头' if macd['trend'] == 'up' else '空头'}")
        lines.append(f"  趋势：{'上涨' if m['direction'] == 'up' else '下跌'}")
        if m['zhongshus']:
            zs = m['zhongshus'][-1]
            lines.append(f"  中枢：{zs['range'][0]:.2f} - {zs['range'][1]:.2f}")
        lines.append(f"  高/低：{l['recent_high']:.2f} / {l['recent_low']:.2f}")
        beichi = find_trend_beichi(m['bis'], m['zhongshus'])
        if beichi:
            lines.append(f"  {'⚠️ 背驰确认' if beichi['beichi'] else '暂无背驰'}：进入{beichi['enter_pct']:.2f}% vs 离开{beichi['leave_pct']:.2f}%")
        if len(m['bis']) >= 3 and name != '日线':
            last_bi, prev_bi = m['bis'][-1], m['bis'][-2]
            if abs(last_bi['change_pct']) < abs(prev_bi['change_pct']):
                lines.append(f"  ⚠️ 盘整背驰：{last_bi['change_pct']:.2f}% < {prev_bi['change_pct']:.2f}%")
        return lines
    
    report.append("一、日线级别分析\n" + "\n".join(level_report(daily_a, '日线')))
    report.append("")
    report.append("二、30分钟级别分析\n" + "\n".join(level_report(m30_a, '30分钟')))
    report.append("")
    report.append("三、5分钟级别分析\n" + "\n".join(level_report(m5_a, '5分钟')))
    report.append("")
    report.append("四、1分钟级别分析\n" + "\n".join(level_report(m1_a, '1分钟')))
    report.append("")
    ...
    if all([daily_a, m30_a, m5_a, m1_a]):
        ...
        for k, v in dirs.items():
            report.append(f"  {k}：{'上涨' if v == 'up' else '下跌'}")
    ...
    if daily_a and m30_a:
        ...
        if d_dir == 'down':
            report.append(f"    下跌延续：考验{d_low:.2f}支撑")
            report.append(f"    操作：空头持有或反弹后做空")
        else:
            report.append(f"    上涨延续：测试{d_high:.2f}阻力")
            report.append(f"    操作：多头持有，关注突破")
        report.append(f"  【分类二】（中概率 ~30%）")
        report.append(f"    震荡整理：{d_low:.2f} - {d_high:.2f}区间波动")
        report.append(f"    操作：高抛低吸，突破后跟进")
    ...
    report.append("-" * 40)
    if m30_a and daily_a:
        current = m30_a['levels']['current']
        d_high, d_low = daily_a['levels']['recent_high'], daily_a['levels']['recent_low']
        if m30_a['direction'] == 'up':
            report.append("  持有多单：")
            report.append(f"    持有，止盈参考：{d_high:.2f}")
            report.append(f"    离场：1分钟顶背驰+跌破均线")
            report.append("  空仓/想做多：")
            report.append("    等待回调结束信号")
        else:
            report.append("  持有多单：")
            report.append(f"    注意风控，止损：{d_high:.2f}上方")
            report.append("  空仓/想做空：")
            report.append(f"    反弹衰竭时做空，理想区域：{d_high:.2f}")
            report.append(f"    止损：突破{d_high:.2f}严格止损")
    report.append("")
