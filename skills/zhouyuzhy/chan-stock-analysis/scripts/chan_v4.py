...
def get_data_futu(code, period):
    ...
    try:
        ...
        futu_code = f"SZ.{code}" if not code.startswith('SH') and not code.startswith('SZ') else code
        if not ('.' in futu_code):
            futu_code = f"SZ.{code}"
        ...
        with OpenQuoteContext(host='127.0.0.1', port=11111) as ctx:
            result = ctx.request_history_kline(futu_code, ktype=period_map[period], max_count=500)
...
def get_data_tushare(code, period):
    ...
    try:
        ...
        TOKEN = "38d141546ad7a95940b8f3ca3dcbdf5184b936c8ce517eeed9d647e6"
        ...
        freq_map = {'daily': 'D', '30': '30', '5': '5', '1': '1'}
...
def get_kline(code, period, is_index=False):
    """按优先级获取K线数据"""
    # 首选 akshare
...
def calculate_fibonacci(klines, lookback=30):
    ...
    # 计算回撤位（从低点到高点）
    fib_levels = {
        '0.236': min_low + diff * 0.236,
        '0.382': min_low + diff * 0.382,
        '0.500': min_low + diff * 0.500,
        '0.618': min_low + diff * 0.618,
    }
...
def format_fib(fib):
    ...
    lines = []
    lines.append(f"  斐波那契回撤位（{fib['trend']}趋势）：")
    lines.append(f"    区间：{fib['low']:.2f} - {fib['high']:.2f}")
    lines.append(f"    当前位置：{fib['position']*100:.1f}%")
    lines.append(f"    关键位：")
    for level in ['0.618', '0.500', '0.382', '0.236']:
        price = fib['levels'][level]
        marker = ""
        if fib['support'] and fib['support'][0] == level:
            marker = " ←支撑"
        elif fib['resistance'] and fib['resistance'][0] == level:
            marker = " ←阻力"
...
def generate_report(stock_name, stock_code, daily, m30, m5, m1):
    ...
    if m30_a:
        if resonance == 'up':
            ...
            report.append(f"  空仓想做多：回调至斐波那契支撑位入场")
            report.append(f"  做空：暂不推荐，共振做多")
        elif resonance == 'down':
            report.append(f"  持有多单：注意止损")
            report.append(f"  空仓想做空：反弹至斐波那契阻力位做空")
...
def main():
    parser = argparse.ArgumentParser(description='缠论多级别联立分析 v4')
    ...
    print(f"  策略：akshare → futu → tushare")
    print(f"{'='*60}\n")
    ...
    if daily:
        print(f"   成功：{len(daily)}根 (数据源: {daily_src})\n")
    else:
        print("   ❌ 失败\n")
    ...
    if m30:
        print(f"   成功：{len(m30)}根 (数据源: {m30_src})\n")
    else:
        print("   ❌ 失败\n")
    ...
    if m5:
        print(f"   成功：{len(m5)}根 (数据源: {m5_src})\n")
    else:
        print("   ❌ 失败\n")
    ...
    if m1:
        print(f"   成功：{len(m1)}根 (数据源: {m1_src})\n")
    else:
        print("   ❌ 失败\n")
