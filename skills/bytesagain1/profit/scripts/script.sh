#!/usr/bin/env bash
# profit — Profit reference tool. Use when working with profit in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
profit v$VERSION — Profit Reference Tool

Usage: profit <command>
...
## What is Profit?
Profit (profit) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with profit.

## Key Concepts
- Core profit principles and fundamentals
- How profit fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Profit Matters
Understanding profit is critical for:
...
## Getting Started
1. Understand the basic profit concepts
...
## Installation
1. Download or clone the profit package
...
cmd_patterns() {
    cat << 'EOF'
# Profit — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for profit
...
cmd_performance() {
    cat << 'EOF'
# Profit — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Profit — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Profit — Migration & Upgrade Guide
