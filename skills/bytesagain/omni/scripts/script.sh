#!/usr/bin/env bash
# omni — Omni reference tool. Use when working with omni in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
omni v$VERSION — Omni Reference Tool
...
## What is Omni?
Omni (omni) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with omni.

## Key Concepts
- Core omni principles and fundamentals
- How omni fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Omni Matters
Understanding omni is critical for:
...
## Getting Started
1. Understand the basic omni concepts
...
## Installation
1. Download or clone the omni package
...
cmd_patterns() {
    cat << 'EOF'
# Omni — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for omni
...
case "$CMD" in
    ...
    version|--version|-v) echo "omni v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: omni help"; exit 1 ;;
esac
