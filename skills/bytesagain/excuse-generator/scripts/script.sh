#!/usr/bin/env bash
# excuse-generator - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${EXCUSE_GENERATOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/excuse-generator}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
excuse-generator v$VERSION

Multi-purpose utility tool

Usage: excuse-generator <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "excuse-generator v$VERSION" ;;
