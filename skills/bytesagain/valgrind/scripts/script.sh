#!/usr/bin/env bash
# valgrind — Valgrind reference tool. Use when working with valgrind in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
valgrind v$VERSION — Valgrind Reference Tool

Usage: valgrind <command>
...
## What is Valgrind?
Valgrind (valgrind) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with valgrind.

## Key Concepts
- Core valgrind principles and fundamentals
- How valgrind fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Valgrind Matters
Understanding valgrind is critical for:
...
## Getting Started
1. Understand the basic valgrind concepts
...
## Installation
1. Download or clone the valgrind package
...
cmd_patterns() {
    cat << 'EOF'
# Valgrind — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for valgrind
...
cmd_performance() {
    cat << 'EOF'
# Valgrind — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Valgrind — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Valgrind — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "valgrind v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: valgrind help"; exit 1 ;;
esac
