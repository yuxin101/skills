...
export class AnomalyDetectionModule {
     ...
     * 计算分布差异
     */
    calculateDistributionDifferences(baselineCounts, anomalyCounts, topk = 10) {
        ...
        if (baselineTotal === 0 || anomalyTotal === 0)
            return [];
        // 计算每个值的分布差异
        allKeys.forEach(key => {
     ...
     */
    async executePatternClassification(logReduceResult, limit = 20) {
        try {
            // 验证输入数据
     ...
     */
    analyzePatterns(patterns, totalHits) {
        const analyzedPatterns = patterns.map(pattern => {
            ...
            return {
                ...
                // 综合重要性评分
                significance_score: this.calculateSignificanceScore(pattern, timelineAnalysis, frequencyAnalysis, anomalyAnalysis),
                // 模式分类
     ...
     * 分析模式的时间分布特征
     */
    analyzePatternTimeline(pattern) {
     ...
     * 分析模式的频率特征
     */
    analyzePatternFrequency(pattern, totalHits) {
     ...
     * 检测模式中的异常
     */
    detectPatternAnomalies(pattern) {
        if (!pattern.timeline?.rows) {
            return { has_anomalies: false, anomalies: [] };
        }
        const counts = pattern.timeline.rows.map((row) => row.count || 0);
        // 使用统计学方法检测异常
     ...
     * 分析时间分布特征
     */
    analyzeTemporalDistribution(counts) {
     ...
     * 计算时间聚集度
     */
    calculateTemporalClustering(counts) {
        if (counts.length < 3)
            return 0;
     ...
     * 计算相对重要性
     */
    calculateRelativeImportance(count, total) {
        if (total === 0)
            return 0;
        // 使用对数缩放避免大数值主导
     ...
     * 检测统计异常
     */
    detectStatisticalAnomalies(counts) {
        if (counts.length < 3)
            return [];
        const mean = counts.reduce((a, b) => a + b, 0) / counts.length;
        const variance = counts.reduce((sum, count) => sum + Math.pow(count - mean, 2), 0) / counts.length;
     ...
     * 检测时间异常
     */
    detectTemporalAnomalies(timeline) {
        if (!timeline.rows || timeline.rows.length < 3)
            return [];
        const anomalies = [];
        const rows = timeline.rows;
        // 检测不规则的时间间隔
     ...
     * 计算综合重要性评分
     */
    calculateSignificanceScore(pattern, timelineAnalysis, frequencyAnalysis, anomalyAnalysis) {
        let score = 0;
        // 频率权重 (30%)
     ...
     * 分类模式
     */
    classifyPattern(pattern, timelineAnalysis, anomalyAnalysis) {
     ...
     * 生成模式分析摘要
     */
    generatePatternSummary(analyzedPatterns, totalHits) {
     ...
     * 生成关键洞察
     */
    generateKeyInsights(analyzedPatterns, classifications) {
        const insights = [];
     ...
     */
    async executePeriodCompareWithData(timeSeriesA, timeSeriesB, options = {}) {
        try {
            ...
            if (compare_fields.length > 0) {
                // 使用提供的时间范围参数进行字段对比
                fieldDifferences = await this.compareFields(query, options.time_range_a || '', options.time_range_b || '', options.index_name || 'yotta', compare_fields, topk);
            }
            const seriesA = pointsA.map((point) => ({
                timestamp: String(point.time),
                value: point.count || 0,
                count: point.count || 0
            }));
            const seriesB = pointsB.map((point) => ({
                timestamp: String(point.time),
                value: point.count || 0,
                count: point.count || 0
            }));
    ...
    async executePeriodCompare(params) {
        try {
            const { query = '*', time_range_a, time_range_b, index_name = 'yotta', bucket = '5m', compare_fields = [], topk = 10, metric_field } = params;
            ...
            let fieldDifferences = [];
            if (compare_fields.length > 0) {
                fieldDifferences = await this.compareFields(query, time_range_a, time_range_b, index_name, compare_fields, topk);
            }
            const seriesA = pointsA.map(point => ({
                timestamp: String(point.time),
                value: point.count || 0,
     ...
     * 对比字段分布
     */
    async compareFields(query, timeRangeA, timeRangeB, indexName, fields, topk) {
     ...
     * 获取字段分布
     */
    async getFieldDistribution(query, timeRange, indexName, field) {
        ...
        const distribution = {};
        result.data.fields[field].top_values.forEach((item) => {
    ...
    async executeCorrelationAnalysis(params) {
        try {
            const { query = '*', time_range, index_name = 'yotta', fields = [], method = 'mixed', limit = 50 } = params;
            if (fields.length < 2) {
                return {
                    error: '字段不足',
     ...
     * 计算共现关系
     */
    async calculateCooccurrence(data, fields, limit) {
    ...
    async executeRootCauseSuggestions(params) {
        try {
            const { query = '*', anomaly_window, baseline_window, index_name = 'yotta', candidate_fields = [], significance_threshold = 0.1, topk = 5 } = params;
            ...
            for (const field of fieldsToAnalyze) {
                const baselineFieldData = baselineFields.data?.fields?.find((f) => f.name === field);
                const anomalyFieldData = anomalyFields.data?.fields?.find((f) => f.name === field);
                ...
                const anomalyCounts = {};
                baselineData.top_values.forEach((item) => {
                    baselineCounts[item.value] = item.count;
                });
                anomalyData.top_values.forEach((item) => {
                    anomalyCounts[item.value] = item.count;
                });
                // 计算分布差异
                const differences = this.calculateDistributionDifferences(baselineCounts, anomalyCounts, topk);
                // 过滤显著差异
     ...
     * 获取两个字段列表的交集，用于根因分析
     */
    getCommonFields(baselineFields, anomalyFields) {
     ...
     * 生成根因假设
     */
    generateRootCauseHypothesis(field, topChanges) {
     ...
     * 生成建议查询
     */
    generateSuggestedQueries(baseQuery, suggestions) {
        const queries = [];
        // 生成针对主要变化值的查询
     ...
     * 生成根因总结
     */
    generateRootCauseSummary(suggestions, originalQuery) {
    ...
    async executeAnomalyPoints(params) {
        ...
        return statistics.executeAnomalyPoints(params.query || '*', params.time_range, params.index_name || 'yotta', params.bucket, params.metric_field, params.method || 'zscore', params.sensitivity || 3, params.min_support || 0);
    }
    /**
     * 趋势概要
     */
    async executeTrendSummary(params) {
        // 使用统计模块的趋势分析功能
        const statistics = new StatisticsModule(this.client);
        return statistics.executeTrendSummary(params.query || '*', params.time_range, params.index_name || 'yotta', params.bucket, params.metric_field, params.limit_peaks || 3);
    }
    /**
     * 数据概览
     */
    async executeDataOverview(params) {
        // 使用统计模块的数据概览功能
        const statistics = new StatisticsModule(this.client);
        return statistics.executeDataOverview(params.query || '*', params.time_range, params.index_name || 'yotta', params.metric_field, params.percentiles || [50, 90, 99]);
    }
    /**
     * 趋势预测
     */
...
// 为了向后兼容，导出模块实例创建函数
export function createAnomalyDetectionModule(client) {
    return new AnomalyDetectionModule(client);
}
export function createTrendForecastModule(client) {
    return new TrendForecastModule(client);
}
export function createStatisticsModule(client) {
    return new StatisticsModule(client);
}
export function createLogSearchModule(client) {
