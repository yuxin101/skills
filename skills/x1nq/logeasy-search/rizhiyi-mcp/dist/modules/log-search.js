export class LogSearchModule {
     ...
     * 生成精准溯源链接
     */
    generateQuickLinks(row, originalQuery, timeRange, indexName) {
        const links = {};
        if (!this.baseURL)
            return links;
        // 关键唯一标识字段 - 直接查询该ID
        ...
        if (indexName)
            baseParams.append('index_name', indexName); // 注意：Web端参数可能不叫index_name，通常是datasets或隐含
     ...
     * 执行日志搜索表格API调用
     */
    async executeLogSearchSheet(query, timeRange, indexName = "yotta", limit = 100) {
        try {
     ...
     * 执行日志聚类分析
     */
    async executeLogReducePattern(query, timeRange, indexName = "yotta", patternOptions = {}) {
        try {
            ...
            const result = await this.client.get(apiPath, requestData);
            if (result.error) {
                return result;
            }
            return {
     ...
     * 获取日志聚类分析结果
     */
    async executeLogReducePreview(sid, maxRetries = 10, retryInterval = 5000) {
        try {
            const apiPath = `/api/v3/search/preview/logreduce/`;
            // 轮询获取结果
            const result = await this.client.pollUntilComplete(apiPath, (response) => {
                // 检查是否完成
     ...
     * 列出所有日志字段
     */
    async executeListFields(query, timeRange, indexName = "yotta") {
        try {
            const apiPath = '/api/v3/search/sheets/';
            const params = {
                query,
            ...
            if (data?.results?.fields) {
                fields = data.results.fields.map((fieldInfo) => ({
                    name: fieldInfo.name,
     ...
     * 列出指定字段的所有值及其出现频率
     */
    async executeListFieldValues(field, query, timeRange, indexName = "yotta", limit = 100) {
        try {
            ...
            if (data?.results?.sheets?.rows) {
                values = data.results.sheets.rows.map((row) => ({
                    value: row[field], // 字段值
                    count: row.count // 计数
                }));
                total = values.length;
     ...
     */
    async executeTimeSeriesCounts(query, timeRange, indexName = "yotta", bucket = "5m", metricField) {
        try {
            // 构建timechart查询
            let tsQuery;
            if (metricField) {
            ...
            // 转换API响应格式到期望的格式
            const points = data.results.sheets.rows.map((item) => ({
    ...
    async executeDataOverview(query, timeRange, indexName = "yotta", metricField, percentiles = [50, 90, 99]) {
        try {
            ...
            const total = summary.data?.results?.total_hits ?? 0;
