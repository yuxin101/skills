...
// 工具处理函数
async function handleLogSearchSheet(params) {
    const result = await logSearchModule.executeLogSearchSheet(params.query || "*", params.time_range, params.index_name || "yotta", params.limit || 100);
    return formatResult(result);
}
async function handleLogReducePattern(params) {
    const result = await logSearchModule.executeLogReducePattern(params.query || "*", params.time_range, params.index_name || "yotta", params.pattern_options || {});
    return formatResult(result);
}
async function handleLogReducePreview(params) {
    const result = await logSearchModule.executeLogReducePreview(params.sid, params.max_retries || 10, params.retry_interval || 5000);
    return formatResult(result);
}
async function handleListFields(params) {
    const result = await logSearchModule.executeListFields(params.query || "*", params.time_range, params.index_name || "yotta");
    return formatResult(result);
}
async function handleListFieldValues(params) {
    const result = await logSearchModule.executeListFieldValues(params.field, params.query || "*", params.time_range, params.index_name || "yotta", params.limit || 100);
    return formatResult(result);
}
async function handleDataOverview(params) {
    const result = await statisticsModule.executeDataOverview(params.query || "*", params.time_range, params.index_name || "yotta", params.metric_field, params.percentiles || [50, 90, 99]);
    return formatResult(result);
}
async function handleTrendSummary(params) {
    const result = await statisticsModule.executeTrendSummary(params.query || "*", params.time_range, params.index_name || "yotta", params.bucket, params.metric_field, params.limit_peaks || 3);
    return formatResult(result);
}
async function handleAnomalyPoints(params) {
    const result = await statisticsModule.executeAnomalyPoints(params.query || "*", params.time_range, params.index_name || "yotta", params.bucket, params.metric_field, params.method || 'zscore', params.sensitivity || 3, params.min_support || 0);
    return formatResult(result);
}
async function handlePatternClassification(params) {
    // 如果提供了前一个tool的结果数据，直接使用它进行分析
    if (params.previous_result) {
        const result = await anomalyDetectionModule.executePatternClassification(params.previous_result, params.limit || 20);
        return formatResult(result);
    }
    // 否则执行完整的聚类分析流程
...
async function handlePeriodCompare(params) {
    ...
    if (params.previous_time_series_a || params.previous_time_series_b) {
        ...
        const result = await anomalyDetectionModule.executePeriodCompareWithData(mockResultA, mockResultB, {
...
async function handleCorrelationAnalysis(params) {
...
async function handleRootCauseSuggestions(params) {
...
async function handleTrendForecast(params) {
...
async function handleAnomalyAlert(params) {
