...
main() {
    ...
    TEMP_FILE="$TEMP_DIR/interactive_${TIMESTAMP}.png"
    FINAL_NAME="interactive_${TIMESTAMP}.png"
    
    echo "🖱️  请选择要截图的区域..."
    echo "💡 使用鼠标拖拽选择区域，按 ESC 取消"
    echo ""
    
    # 执行交互式截图
    if /usr/sbin/screencapture -i -t png "$TEMP_FILE"; then
        # 检查文件是否真的被创建了（用户可能取消了）
