...
clawhub publish ./tiktok-video-maker --slug tiktok-video-maker --name "TikTok Video Maker" --version 1.0.1 --changelog "Initial release — avatar video generation for OpenClaw agents"
```
...
> "Just published a video generation skill to ClawHub — lets your agent generate avatar videos from a script. Built on LovelyBots API. Install with `npx clawhub add tiktok-video-maker`"
...
```bash
clawhub publish . --slug tiktok-video-maker --version 1.0.1 --changelog "Updated polling docs and added aspect ratio support"
```
