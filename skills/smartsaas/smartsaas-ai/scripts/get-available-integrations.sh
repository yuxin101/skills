#!/usr/bin/env bash
# Usage: get-available-integrations.sh
# Requires: SMARTSAAS_BASE_URL, SMARTSAAS_API_KEY
# Lists available integrations for product sync.
set -e
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X GET "${BASE}/api/protected/products/sync/available-integrations" -H "Authorization: Bearer $KEY")
