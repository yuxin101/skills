#!/usr/bin/env bash
# Usage: refresh-user.sh
# Requires: SMARTSAAS_BASE_URL, SMARTSAAS_API_KEY
# Refreshes the current user session/data from the backend.
set -e
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X GET "${BASE}/api/protected/user/refresh-user" -H "Authorization: Bearer $KEY")
