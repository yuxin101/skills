#!/usr/bin/env bash
# Usage: push-products-to-integration.sh <dataId> <integrationType>
# Requires: SMARTSAAS_BASE_URL, SMARTSAAS_API_KEY
# Pushes products from a data folder to an integration (e.g. shopify).
set -e
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/protected/products/sync/push/${DATA_ID}" \
...
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Push to $INTEGRATION_TYPE completed for data $DATA_ID."
  echo "$BODY_RES"
else
