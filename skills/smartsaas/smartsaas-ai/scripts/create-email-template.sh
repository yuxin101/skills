#!/usr/bin/env bash
# Usage: create-email-template.sh <templateJson>
#   Shape: smartsaas-backend/src/schema/TemplateModels/Email.mjs (EmailTemplateSchema). Required: title. Optional: content, description, tags, projectData, global, standardTemplateType, dynamicText.
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/protected/templates/post-email-template" \
