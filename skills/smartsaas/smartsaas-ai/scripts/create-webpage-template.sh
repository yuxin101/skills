#!/usr/bin/env bash
# Usage: create-webpage-template.sh <templateJson>
#   Shape: smartsaas-backend/src/schema/TemplateModels/Webpage.mjs (WebpageTemplateSchema). Required: title. Optional: description, content, tags, urlSlug, isPublished, global.
...
DATA="${1:-{}}"
if [ -z "$DATA" ]; then
  echo "Error: template data JSON (first argument) is required."
  exit 1
fi
