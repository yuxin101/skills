...
def _get_version_via_requests(api_url: str) -> str | None:
    ...
    if resp.status_code != 200:
        return None
    return _extract_version(resp.json())


def _get_version_via_clawhub(slug: str) -> str | None:
    ...
    if result.returncode != 0:
        return None
    return _extract_version(json.loads(result.stdout))
...
def get_remote_version(slug: str) -> str | None:
    """
    从 ClawHub registry 查询指定 slug 的最新版本号，返回版本字符串或 None。

    三级降级策略：
      L1: requests 直接请求 API（最快，自带 SSL 证书）
      L2: clawhub inspect（本地 CLI）
      L3: npx clawhub inspect（临时下载执行，兜底）
    """
    api_url = f"https://clawhub.ai/api/v1/skills/{urllib.parse.quote(slug, safe='')}"
    strategies = [
        lambda: _get_version_via_requests(api_url),
        lambda: _get_version_via_clawhub(slug),
        lambda: _get_version_via_npx(slug),
    ]
