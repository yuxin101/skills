"""CDP WebSocket 客户端（Browser, Page, Element）。

通过原生 WebSocket 与 Chrome DevTools Protocol 通信，实现浏览器自动化控制。
从 xiaohongshu-skills 提取的通用引擎，100% 平台无关。
"""
...
from .stealth import STEALTH_JS, build_ua_override  # noqa: F401

# 禁用代理，避免系统代理干扰本地 CDP 连接（与 xiaohongshu-skills 相同方案）
...
class Page:
    ...
    def click_element_by_text(self, tag_selector: str, text: str) -> bool:
        """通过元素文字内容点击（用于无 aria-label 的按钮）。

        Args:
            tag_selector: CSS 选择器范围，如 'div[role="button"]'
            text: 精确匹配的 textContent（strip 后比较）

        Returns:
            True 表示找到并点击，False 表示未找到。
        """
        box = self.evaluate(
            f"""
            (() => {{
