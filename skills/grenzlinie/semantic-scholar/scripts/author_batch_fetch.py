...
"""
Semantic Scholar author batch fetch script.

Purpose
-------
Fetch author metadata in batches using:
...
def build_headers() -> Dict[str, str]:
    ...
    headers = {
        ...
        "User-Agent": "semantic-scholar-author-batch/1.0",
    }
    if api_key:
...
def flatten_aliases(aliases: Any) -> str:
    if not isinstance(aliases, list):
        return ""
    return "; ".join(str(value) for value in aliases if value)


def flatten_papers(papers: Any) -> str:
...
def author_to_csv_row(author: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "authorId": author.get("authorId"),
        "name": author.get("name"),
        "aliases": flatten_aliases(author.get("aliases")),
        "paperCount": author.get("paperCount"),
        "citationCount": author.get("citationCount"),
        "hIndex": author.get("hIndex"),
        "homepage": author.get("homepage"),
        "url": author.get("url"),
        "affiliations": json.dumps(author.get("affiliations", []), ensure_ascii=False),
        "papers": flatten_papers(author.get("papers")),
    }
...
def export_csv(jsonl_path: str, csv_output: str) -> None:
    ...
    with open(jsonl_path, "r", encoding="utf-8") as fin:
        for line in fin:
            ...
            if not line:
                continue
            rows.append(author_to_csv_row(json.loads(line)))
...
def fetch_authors(
    ...
    if not ids:
        raise ValueError("No author IDs provided.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch fetch author metadata from Semantic Scholar."
    )
    parser.add_argument("--ids", type=str, default=None, help="Comma-separated author IDs.")
    parser.add_argument(
        "--ids-file",
        type=str,
        default=None,
        help="Text file with one author ID per line.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="author_batch_results.jsonl",
    ...
    parser.add_argument(
        "--fields",
        type=str,
        default="authorId,name,paperCount,citationCount,hIndex,homepage,url,affiliations,papers.title",
