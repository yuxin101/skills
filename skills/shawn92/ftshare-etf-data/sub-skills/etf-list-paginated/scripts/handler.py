#!/usr/bin/env python3
"""ETF 分页列表（market.ft.tech）：分页、排序、筛选"""
import argparse
import json
import sys
import urllib.error
...
def main():
    parser = argparse.ArgumentParser(description="ETF 分页列表，支持排序、筛选、分页")
    parser.add_argument(
        "--order_by",
        default=None,
    ...
    parser.add_argument(
        "--filter",
        default=None,
        help="过滤条件表达式，如 change_rate != null",
    )
    parser.add_argument(
        "--masks",
        default=None,
    ...
    if args.filter_index is not None:
        params["filter_index"] = args.filter_index

    path = "/app/api/v2/etfs"
    if params:
