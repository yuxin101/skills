...
| `config/api_key.txt` | 凭证（单行 `pk:sk` 或粘贴接口 JSON）；可由 `GET /api/v1/api-keys` 手工填入，或由薄客户端自动拉取并写入本文件（见 `config_api_key.txt`） |
...
1. **获取 API Key**（首次无本地凭证时）  
   ...
   **可选**查询参数 **`account_id`**（已有账户 UUID）；设置环境变量 **`ACCOUNT_ID`** / **`PRANA_ACCOUNT_ID`** 时，薄客户端会在请求中自动附加。  
...
3. **环境变量**（可选）
   ...
   - `ACCOUNT_ID` / `PRANA_ACCOUNT_ID`：自动拉 key 时作为 `account_id` 查询参数  
...
1. 检查本地是否已配置 `public_key`/`secret_key`；若无则 `GET /api/v1/api-keys`（可按需带 `account_id`）再写入 **`api_key.txt`**，或依赖薄客户端自动拉取并写 `api_key.txt`。  
2. 根据环境调用 **`prana_skill_client.py`** 或 **`prana_skill_client.js`**（后者需先 `npm install`），将用户消息作为 `-m` 传入，维护 `thread_id` 实现多轮对话。  
3. **`/api/claw/agent-run`** 与 **`/api/claw/agent-result`** 的响应须**原样**交给最终用户（原样展示或原样传递），勿另行改写、摘要或做其它二次处理；渠道若自行调这两个接口，亦同。  
4. 若返回付费链接（HTTP 402 等），按文档引导用户完成支付后重试。
