#!/usr/bin/env python3
"""
UniFuncs Deep Search chat/completions client.
Usage: ./deep-search-chat.py "query" [options]
"""
...
CHAT_COMPLETIONS_ENDPOINT = "https://api.unifuncs.com/deepsearch/v1/chat/completions"
DEFAULT_MODEL = "s3"
DEFAULT_REQUEST_TIMEOUT_SECONDS = 180
DEFAULT_STREAM_TIMEOUT_SECONDS = 900
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="UniFuncs Deep Search client",
    )
    parser.add_argument("query", nargs="?", help="User query sent to Deep Search.")
    parser.add_argument(
        "--model",
        choices=["s1", "s2", "s3"],
    ...
    parser.add_argument(
        "--raw-response",
        action="store_true",
        help="Print full API response JSON instead of concise text.",
    )
...
def create_temp_stream_file() -> Optional[str]:
    ...
    if not os.access(tmp_dir, os.W_OK):
        return None
    fd, path = tempfile.mkstemp(prefix="unifuncs-deep-search-", suffix=".stream", dir=tmp_dir)
    os.close(fd)
    return path
...
def resolve_stream_file_path(specified_path: Optional[str]) -> Optional[str]:
    ...
    if specified_path:
        ...
        if not os.path.isdir(parent_dir):
            raise UniFuncsDeepSearchError(f"Stream file directory does not exist: {parent_dir}")
        if not os.access(parent_dir, os.W_OK):
            raise UniFuncsDeepSearchError(f"Stream file directory is not writable: {parent_dir}")
...
def extract_text_from_stream_file(stream_file_path: str) -> str:
    ...
    if not os.path.isfile(stream_file_path):
        raise UniFuncsDeepSearchError(f"Stream file does not exist: {stream_file_path}")
    ...
    try:
        ...
        raise UniFuncsDeepSearchError(f"Failed to read stream file: {err}") from err
...
def start_background_worker(args: argparse.Namespace, stream_file_path: str) -> None:
    ...
    if args.important_urls:
        cmd.extend(["--important-urls", args.important_urls])
    if args.important_keywords:
        cmd.extend(["--important-keywords", args.important_keywords])
    if args.important_prompt:
        cmd.extend(["--important-prompt", args.important_prompt])
    if args.introduction:
...
def stream_chat(
    ...
    """Stream chat response and persist raw chunks to temp file when writable."""
    req = urllib.request.Request(
    ...
    try:
        if args.read_stream_file:
            ...
            if cleanup_stream_file_if_done(stream_file_path):
                print(f"[已清理] 流文件已完成并删除：{stream_file_path}", file=sys.stderr)
            return
        ...
        if args.stream:
            ...
            if not temp_path:
                raise UniFuncsDeepSearchError("No writable stream file available for streaming mode.")

            if args.background_worker:
                # Background worker performs the full streaming request and exits only when stream is done.
            ...
            if not is_stream_done(temp_path):
                ...
                notice_lines = [
                    "",
                    "",
                    f"[未完成] 在 {args.timeout}s 内未收到完整响应，仅返回已成功接收的部分。",
                    "[后台继续] 流式接收仍在后台进行，你可稍后继续读取。",
                    f"[流文件] {temp_path}",
                    "[后续读取] 使用以下命令继续读取已接收内容：",
                    command,
                ]
                output += "\n".join(notice_lines)
            print(output)
        ...
        print(f"Deep Search failed: {err}", file=sys.stderr)
        sys.exit(1)
