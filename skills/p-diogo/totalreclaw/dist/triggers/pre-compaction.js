...
exports.preCompaction = preCompaction;
 ...
 * ```
 */
async function preCompaction(context, options) {
    const startTime = Date.now();
    if (options.debug) {
        (0, debug_1.debugLog)(true, `Pre-compaction hook started`);
        (0, debug_1.debugLog)(true, `Analyzing ${context.history.length} turns`);
    }
    try {
        // Run comprehensive extraction
        const result = await runComprehensiveExtraction(context, options);
        if (options.debug) {
            (0, debug_1.debugLog)(true, `Pre-compaction completed in ${result.processingTimeMs}ms`);
            (0, debug_1.debugLog)(true, `  Extracted: ${result.factsExtracted}`);
            (0, debug_1.debugLog)(true, `  Stored: ${result.factsStored}`);
            (0, debug_1.debugLog)(true, `  Skipped: ${result.duplicatesSkipped}`);
            if (result.graphStats) {
                (0, debug_1.debugLog)(true, `  Entities: ${result.graphStats.uniqueEntities}`);
                (0, debug_1.debugLog)(true, `  Relations: ${result.graphStats.uniqueRelations}`);
            }
        }
        return {
 ...
 */
async function runComprehensiveExtraction(context, options) {
    const startTime = Date.now();
    const result = {
        factsExtracted: 0,
        factsStored: 0,
    ...
    try {
        ...
        (0, debug_1.debugLog)(!!options.debug, `Raw extraction: ${result.factsExtracted} facts`);
        // Step 3: Get existing memories for deduplication
        const existingMemories = await getExistingMemories(context, options);
        (0, debug_1.debugLog)(!!options.debug, `Found ${existingMemories.length} existing memories for deduplication`);
        // Step 4: Create deduplicator and process facts
        const deduplicator = (0, extraction_1.createDeduplicator)(options.llmClient, options.vectorStoreClient, {
            similarityThreshold: 0.85,
            topK: 5,
            useLLMJudge: true,
        });
        // Step 5: Deduplicate extracted facts
        const deduplicatedFacts = await deduplicator.deduplicateFacts(extractionResult.facts, existingMemories);
        ...
        if (options.consolidateGraph !== false) {
            ...
            (0, debug_1.debugLog)(!!options.debug, `Graph consolidation: ${result.graphStats.entitiesMerged} entities merged`);
        }
 ...
 */
async function getExistingMemories(context, options) {
    try {
        ...
        return results.map(r => {
            ...
            const typeTag = r.fact.metadata?.tags?.find(t => ['fact', 'preference', 'decision', 'episodic', 'goal'].includes(t));
            const type = typeTag || 'fact';
            return {
                id: r.fact.id,
 ...
 */
function buildDeduplicationQuery(context) {
 ...
 */
async function consolidateGraph(facts, options) {
    const stats = {
        entitiesMerged: 0,
        relationsMerged: 0,
        uniqueEntities: 0,
 ...
 */
async function storeFactsBatch(facts, options) {
    const result = {
        stored: 0,
        skipped: 0,
        updates: 0,
 ...
 */
async function storeFactWithResult(fact, options) {
