#!/usr/bin/env python3
"""Extract structured ride-insights records from emails.json via OpenClaw Gateway."""
...
def is_local_gateway_host(hostname: str | None) -> bool:
    if not hostname:
        return False
...
def load_gateway_settings():
    ...
    if not is_local_gateway_host(parsed.hostname):
        raise RuntimeError(
            f'Refusing to send ride email content to non-local Gateway host {parsed.hostname}. '
            'This skill only supports a local loopback Gateway (localhost/127.0.0.1/::1).'
        )
...
def main():
    ap = argparse.ArgumentParser(description='Extract structured rides for ride-insights via OpenClaw Gateway')
    ap.add_argument('--emails-json', required=True, help='Path to the fetched ride receipt emails.json file')
    ap.add_argument('--out', required=True, help='Output path for extracted rides.json')
    ap.add_argument('--limit', type=int, default=DEFAULT_LIMIT, help='Maximum number of emails to extract; defaults to 50')
    ap.add_argument('--delay-ms', type=int, default=0, help='Optional delay between extraction requests in milliseconds')
    args = ap.parse_args()
