...
```bash
mkdir -p skills
git clone https://github.com/BitsLabSec/Claw_Wallet_Skill.git skills/claw-wallet
...
Minimal local-wallet verification:

1. `GET {CLAY_SANDBOX_URL}/health`
2. poll `GET {CLAY_SANDBOX_URL}/api/v1/wallet/status`
...
Pull the latest code from the repo and rerun the installer. Local changes (e.g. in tracked files) are stashed before `git pull` and restored afterward. Wallet data (`.env.clay`, `identity.json`, `share3.json`) is preserved.
...
- `& "skills/claw-wallet/claw-wallet.ps1" policy get`
