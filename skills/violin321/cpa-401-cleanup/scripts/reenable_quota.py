...
def main():
    ap = argparse.ArgumentParser(description="Re-enable quota accounts from exported JSON")
    ...
    print(f"about to re-enable quota accounts: {len(names)}")
    if names[:10]:
        for s in names[:10]:
            print(f"  - {s}")
    if not args.yes:
    ...
    for name in names:
        payload = json.dumps({"name": name, "disabled": False}).encode("utf-8")
        req = urllib.request.Request(url, headers=headers(token), method="PATCH", data=payload)
        try:
    ...
    summary = {"requested": len(names), "reenabled": ok, "failed": len(failed), "failures": failed[:20]}
