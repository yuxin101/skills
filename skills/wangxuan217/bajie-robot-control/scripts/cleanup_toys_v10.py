...
# -*- coding: utf-8 -*-
"""
八界机器人 - 玩具收纳区整理玩具（v4 - 使用 search 搜索收纳筐）

工作流程（严格串行）：
0. robot_prepare_pose → 等待 finish
1. search(收纳筐 @ 地上) → 等待 finish
   - 成功：系统记录容器位置，继续
   - 失败：尝试 search(收纳筐 @ 客厅)
...
async def search_container(ws, container_name: str, area_name: str) -> tuple:
    ...
    使用 search 接口（比 search_container 更稳定）
    系统会自动记录容器位置，后续 search 会过滤容器内物体
    """
    print(f"\n{'='*70}")
    print(f"【步骤 1】搜索收纳筐 (search)")
    print(f"{'='*70}")
...
async def main():
    print("=" * 70)
    print("🤖 八界机器人 - 玩具收纳区整理玩具（v4 - 使用 search 搜索收纳筐）")
    print("=" * 70)
    ...
    async with websockets.connect(WEBSOCKET_URL) as ws:
        ...
        for area in ["地上", "客厅"]:
            print(f"\n>>> 尝试搜索区域：{area} <<<")
            container_found, container_data = await search_container(ws, "收纳筐", area)
            if container_found:
                print(f"✅ 在{area}找到收纳筐")
                break
            else:
                print(f"⚠️  在{area}未找到收纳筐")
