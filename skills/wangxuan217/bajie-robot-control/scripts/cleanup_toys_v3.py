...
# 整个流程使用相同的 task_id
TASK_ID = f"cleanup_toys_{uuid.uuid4().hex[:8]}"

def generate_uuid() -> str:
    return str(uuid.uuid4())

def build_request(name: str, data: dict) -> dict:
    """构建请求消息 - 使用统一的 TASK_ID"""
    return {
        "header": {
            "mode": "mission",
            "type": "request",
...
async def accurate_grab(ws, search_data: Dict) -> bool:
    ...
    request = build_request("accurate_grab", grab_data)
    await ws.send(json.dumps(request))
...
async def semantic_place(ws, object_name: str, container_name: str) -> bool:
    ...
    request = build_request("semantic_place", {
        "area_id": "",
