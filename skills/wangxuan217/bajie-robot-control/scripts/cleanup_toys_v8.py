...
# -*- coding: utf-8 -*-
"""
八界机器人 - 玩具收纳区整理玩具（v2 - search_container 优化版）

工作流程：
0. robot_prepare_pose(mission_summary: "玩具收纳")
1. search_container 搜索收纳筐 → 系统自动记录容器位置
2. 循环搜索玩具 → 抓取 → 放置
3. 连续 3 次没找到 → 完成
4. robot_ending_pose

参数格式（已验证）：
- search: object + area (单对象)
- object: {item, color, shape, person, type[], subtype[]}
...
async def search_container(ws, container_name: str, area_name: str) -> Optional[Dict]:
    ...
    await ws.send(json.dumps(request))
    result = await wait_for_finish(ws, f"搜索容器{container_name}")
    
    if result["success"]:
        print(f"💾 系统已记录容器位置，后续 search 将自动过滤")
        return result["search_data"]
    else:
        print(f"⚠️  未找到容器，将继续执行（使用默认过滤）")
        return None
...
async def search_multi_area(ws, object_name: str, areas: list) -> Optional[Dict]:
    for area in areas:
...
async def main():
    print("=" * 70)
    print("🤖 八界机器人 - 玩具收纳区整理玩具（v2 - search_container）")
    print("=" * 70)
    ...
    async with websockets.connect(WEBSOCKET_URL) as ws:
        ...
        # 步骤 1: 搜索收纳筐
        print("\n" + "=" * 70)
        print("【步骤 1】搜索收纳筐 (search_container)")
        print("=" * 70)
        await search_container(ws, "收纳筐", "客厅")
        await asyncio.sleep(1)
