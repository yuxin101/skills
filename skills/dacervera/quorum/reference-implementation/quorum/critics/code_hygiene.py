...
"""
Code Hygiene Critic — Structural quality, maintainability, and reliability analysis.

Evaluates Python and PowerShell code for issues that require LLM semantic judgment —
things that deterministic pre-screen checks (PS-001 through PS-010) cannot reliably
assess from syntax alone:

Architecture
────────────
Quorum separates code evaluation into two complementary critics:

  • **Code Hygiene Critic** (this file) — structural quality: correctness, error
    handling, resource management, complexity, duplication, naming, async patterns,
    type safety, import hygiene, portability, testability, and agentic code patterns.
    Primary quality models: ISO/IEC 25010:2023 (Maintainability + Reliability
    sub-characteristics) and ISO/IEC 5055:2021 (CISQ ASCMM/ASCRM CWE mappings).

  • **Security Critic** (security.py) — attack resistance: injection, authentication,
    authorization, cryptography, secrets handling, and SSRF.  Primary frameworks:
    OWASP ASVS 5.0, CWE Top 25, NIST SA-11.

Delegation boundary: when this critic encounters security-adjacent patterns (eval/exec,
Invoke-Expression, credential literals, prompt injection vectors), it flags the pattern
for traceability and **delegates** to the Security Critic for security-specific severity,
exploitability, and remediation assessment.  Neither critic duplicates the other.

Two-Layer Evaluation Model
──────────────────────────
Layer 1 — Deterministic (runs before this critic):
  Custom regex checks (PS-001–PS-010) emit pre-verified PASS/FAIL/SKIP results
  that are injected into the critic prompt as a structured evidence block.  The LLM
  must NOT repeat findings already confirmed by the pre-screen.

Layer 2 — LLM Judgment (this critic):
  Semantic and design-quality issues that static analysis cannot assess:
  - Business logic correctness and off-by-one reasoning
  - Completeness and adequacy of exception handling
  - Context manager lifecycle across all exception paths
  - Race conditions and async/sync boundary errors
  - Cohesion, layer violations, God class/function patterns
  - Naming clarity beyond convention enforcement
  - Docstring accuracy (not just presence)
  - None/Optional propagation and type annotation correctness
  - Testability: purity, side effects, injection design
  - Hardcoded environment assumptions and portability gaps
  - Agentic patterns: prompt construction, retry logic, LLM API hygiene,
    pipeline error handling, credential management, observability

Evaluation Categories (from CODE_HYGIENE_FRAMEWORK.md)
───────────────────────────────────────────────────────
  CAT-01  Code Correctness      ISO 25010: Reliability → Faultlessness
  CAT-02  Error Handling        ISO 25010: Reliability → Fault Tolerance
  CAT-03  Resource Management   ISO 25010: Performance Efficiency → Resource Utilization
  CAT-04  Complexity & Modularity  ISO 25010: Maintainability → Analysability/Modularity
  CAT-05  Code Duplication      ISO 25010: Maintainability → Reusability
  CAT-06  Naming & Documentation   ISO 25010: Maintainability → Analysability
  CAT-07  Type Safety           ISO 25010: Reliability → Faultlessness
  CAT-08  Async & Concurrency   ISO 25010: Reliability → Fault Tolerance
  CAT-09  Import Hygiene        ISO 25010: Maintainability → Modularity
  CAT-10  Style & Formatting    ISO 25010: Maintainability → Analysability
  CAT-11  Portability           ISO 25010: Flexibility → Adaptability
  CAT-12  Testability           ISO 25010: Maintainability → Testability

  AP-01   Prompt Construction   (Agentic)
  AP-02   LLM API Call Patterns (Agentic)
  AP-03   Agent Pipeline Errors (Agentic)
  AP-04   Credential Management (Agentic — flags only; delegates to SecurityCritic)
  AP-05   Timeout & Retry Logic (Agentic)
  AP-06   Logging & Observability (Agentic)

Evidence Requirement
────────────────────
Every finding MUST quote the specific code excerpt that grounds the claim, OR reference
a pre-screen check ID (e.g. [PS-007]) that pre-verified the issue.  Ungrounded findings
are rejected by the base class _parse_findings() validation step.
"""
...
# Keywords used to filter rubric criteria relevant to code hygiene
_HYGIENE_KEYWORDS: frozenset[str] = frozenset({
    # Core correctness / reliability
    "correct", "correctness", "logic", "error", "exception", "handling",
    "fault", "tolerance", "faultless", "reliability", "robust",
    # Resource / async
    "resource", "async", "concurrency", "concurrent", "thread", "lock",
    "timeout", "retry", "context manager", "leak", "blocking",
    # Structure / maintainability
    "complex", "complexity", "modular", "modularity", "coupling", "cohesion",
    "duplication", "duplicate", "reuse", "reusability",
    # Naming / docs
    "naming", "name", "documentation", "docstring", "comment", "readability",
    "analysability", "self-descriptive",
    # Type / data
    "type", "typing", "annotation", "type safety", "none", "optional",
    # Import / dependency
    "import", "dependency", "circular",
    # Style
    "style", "formatting", "format",
    # Portability
    "portable", "portability", "compatibility", "platform",
    # Testability
    "test", "testability", "testable", "mock", "fixture",
    # Agentic
    "prompt", "llm", "agent", "pipeline", "logging", "observability",
    "credential", "secret", "retry", "backoff",
    # Hygiene catch-all
    "hygiene", "quality", "maintainability", "maintainable",
})


class CodeHygieneCritic(BaseCritic):
    """
    Evaluates structural code quality — the LLM judgment layer on top of deterministic
    SAST pre-screening.

    Scope
    ─────
    Correctness (logic bugs, off-by-one), error handling completeness, resource lifecycle
    management, complexity and cohesion, code duplication, naming clarity, type annotation
    correctness, async/concurrency patterns, import and dependency hygiene, portability
    assumptions, testability, and agentic code patterns (AP-01 through AP-06).

    Delegation Boundary
    ───────────────────
    When this critic detects security-adjacent patterns — eval()/exec(), Invoke-Expression,
    hardcoded credentials, prompt injection vectors — it emits a finding that documents the
    pattern and explicitly delegates security severity assessment to the Security Critic.
    This critic does NOT assess exploitability, attack surface, or remediation for security
    issues; it only flags the hygienic concern (e.g., "eval() usage detected — code
    hygiene concern: dynamic execution is hard to audit and test; security assessment
    delegated to SecurityCritic").

    Uses Tier 2 model by default (inherits from BaseCritic.evaluate).  Evidence must be
    direct code quotes or explicit pre-screen check ID references.
    """
    ...
    def system_prompt(self) -> str:
        return """You are the Code Hygiene Critic for Quorum, a rigorous multi-critic quality validation system.
...
━━━ DO NOT ━━━
  ...
  • Flag the same issue in multiple findings — one finding per distinct issue instance"""
    ...
    def build_prompt(self, artifact_text: str, rubric: Rubric) -> str:
        """
        Build the code hygiene evaluation prompt.

        Filters rubric criteria to hygiene-relevant entries, injects pre-screen
        evidence if provided via extra_context, and directs the LLM to focus on
        semantic issues that deterministic SAST tools cannot assess.

        Args:
            artifact_text: Full source code text of the artifact under review.
            rubric: The domain rubric supplying evaluation criteria.

        Returns:
            A structured prompt string ready to send to the LLM.

        Raises:
            ValueError: If artifact_text is empty/whitespace or rubric.criteria is empty.
        """
        # V001 input validation (matching SecurityCritic pattern)
        ...
        # Filter rubric criteria relevant to code hygiene domains
        relevant_criteria = [
        ...
        # Fall back to all criteria if keyword filter produces no matches
        if not relevant_criteria:
            logger.warning(
                "CodeHygieneCritic.build_prompt: keyword filter returned 0 matches, "
        ...
        logger.debug(
            "CodeHygieneCritic.build_prompt: selected %d/%d criteria, "
...
### Code Hygiene Criteria to Evaluate
{criteria_text}

## Pre-Screen Evidence

The deterministic pre-screen layer (PS-001 through PS-010: custom regex checks) ran before you
were called. Its structured results will appear in the "Additional Context" section below, if
available.
...
**How to use pre-screen evidence:**
- Checks marked **FAIL** are pre-verified issues — treat them as confirmed facts.
  Add LLM judgment: what is the semantic impact of this finding in this code's context?
  Reference the check ID (e.g. [PS-007]) in your evidence_result field.
- Checks marked **PASS** are confirmed clean — do NOT re-flag them.
- Checks marked **SKIP** are not applicable to this file type — ignore them.

If no pre-screen evidence appears, the pre-screen was disabled or found nothing.
In that case, rely entirely on your semantic analysis of the artifact text.
