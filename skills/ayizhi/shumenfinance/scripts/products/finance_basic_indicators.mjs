...
export async function getFinanceBasicIndicators(input) {
  const instrument = await getInstrumentProfile(input);
  const financeBasicIndicators = await getJson("/api/visualization/finance/basic-indicators", {
  ...
  return {
    instrument,
    finance_basic_indicators: financeBasicIndicators
  };
}
