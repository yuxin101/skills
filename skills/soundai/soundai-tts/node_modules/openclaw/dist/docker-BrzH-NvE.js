import { Do as defaultRuntime, Ea as markOpenClawExecEnv, Ga as resolveUserPath, o as createSubsystemLogger, sa as resolvePathViaExistingAncestorSync, ui as resolveAgentConfig, wi as resolveSessionAgentId } from "./env-D1ktUnAV.js";
import { r as STATE_DIR } from "./paths-CjuwkA2v.js";
...
import { W as getBlockedNetworkModeReason } from "./zod-schema.agent-runtime-DNndkpI8.js";
import { l as CHANNEL_IDS } from "./registry-bOiEdffE.js";
import { t as formatCliCommand } from "./command-format-CCyUqeuM.js";
import { i as resolveMainSessionKey, n as resolveAgentMainSessionKey, t as canonicalizeMainSessionAlias } from "./main-session-DF6UibWM.js";
...
import { a as resolveCoreToolProfilePolicy, t as CORE_TOOL_GROUPS } from "./tool-catalog-BQWPAtTb.js";
...
//#endregion
//#region src/agents/tool-policy-sandbox.ts
...
function resolveEffectiveSandboxToolPolicyForAgent(cfg, agentId) {
...
function isToolAllowedBySandboxToolPolicy(name, policy) {
	const { blockedByDeny, blockedByAllow } = classifyToolAgainstSandboxToolPolicy(name, policy);
	return !blockedByDeny && !blockedByAllow;
}
function resolveSandboxModeForAgent(cfg, agentId) {
	const defaults = cfg?.agents?.defaults?.sandbox;
	return (cfg && agentId ? resolveAgentConfig(cfg, agentId) : void 0)?.sandbox?.mode ?? defaults?.mode ?? "off";
}
function shouldSandboxSession(mode, sessionKey, mainKey) {
	if (mode === "off") return false;
	if (mode === "all") return true;
	return sessionKey.trim() !== mainKey.trim();
}
function formatEffectiveSandboxToolPolicyBlockedMessage(params) {
	...
	const mainSessionKey = params.cfg?.session?.scope === "global" ? resolveMainSessionKey(params.cfg) : resolveAgentMainSessionKey({
		cfg: params.cfg,
		agentId
	});
	...
	if (!(sessionKey ? shouldSandboxSession(sandboxMode, comparableSessionKey, mainSessionKey) : false)) return;
	...
	if (sandboxMode === "non-main") lines.push(`- Use main session key (direct): ${mainSessionKey}`);
...
//#endregion
export { formatEffectiveSandboxToolPolicyBlockedMessage as A, resolveToolProfilePolicy as B, updateRegistry as C, resolveSandboxScope as D, resolveSandboxConfigForAgent as E, expandPolicyWithPluginGroups as F, SANDBOX_BROWSER_SECURITY_HASH_EPOCH as G, DEFAULT_SANDBOX_COMMON_IMAGE as H, mergeAlsoAllowPolicy as I, compileGlobPatterns as K, stripPluginOnlyAllowlist as L, applyOwnerOnlyToolPolicy as M, buildPluginToolGroups as N, isToolAllowed as O, collectExplicitAllowlist as P, expandToolGroups as R, updateBrowserRegistry as S, resolveSandboxBrowserDockerCreateConfig as T, DEFAULT_SANDBOX_IMAGE as U, DEFAULT_SANDBOX_BROWSER_IMAGE as V, SANDBOX_AGENT_WORKSPACE_MOUNT as W, slugifySessionKey as _, execDockerRaw as a, removeBrowserRegistryEntry as b, readDockerPort as c, validateNetworkMode as d, resolveSandboxHostPathViaExistingAncestor as f, resolveSandboxWorkspaceDir as g, resolveSandboxScopeKey as h, execDocker as i, resolveEffectiveSandboxToolPolicyForAgent as j, resolveSandboxToolPolicyForAgent as k, appendWorkspaceMountArgs as l, resolveSandboxAgentId as m, dockerContainerState as n, readDockerContainerEnvVar as o, splitSandboxBindSpec as p, matchesAnyGlobPattern as q, ensureSandboxContainer as r, readDockerContainerLabel as s, buildSandboxCreateArgs as t, getBlockedBindReason as u, readBrowserRegistry as v, computeSandboxBrowserConfigHash as w, removeRegistryEntry as x, readRegistry as y, normalizeToolName as z };
