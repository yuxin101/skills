...
import { g as readConfigFileSnapshot, x as writeConfigFile } from "./auth-profiles-B5ypC5S-.js";
...
import { n as buildPluginCompatibilityNotices, o as formatPluginCompatibilityNotice } from "./status-B5b8fy8e.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-CmEglGfi.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-XvIxmqix.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CAoGg9mD.js");
		const { logConfigUpdated } = await import("./logging-NlNUan4w.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-j0UDmQth.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-C1f7Qrqe.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-DYsodBef.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-tGjgui7p.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-BSfYqwZ8.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-C771eRA0.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-3dNdKg_D.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BG-Lwhvl.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CQWBzNOq.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-CHtelDpi.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-w9vtT8mE.js");
