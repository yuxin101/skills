...
if (_profile) {
  ...
  if (fs.existsSync(_pPath)) {
    ...
    console.log(`[smtp] Using profile: ${_profile}`);
  } else {
    console.error(`[smtp] Profile not found: ${_pPath}`);
    process.exit(1);
  }
}
...
async function sendEmail(options) {
  ...
  // If only html is provided (no text), set text as a plain text fallback
  // This ensures proper preview in email clients
  if (mailOptions.html && !mailOptions.text) {
    ...
    mailOptions.text = mailOptions.html
      ...
      .replace(/\s+/g, ' ')
      .trim()
      .substring(0, 500); // Limit preview length
  }

  const info = await transporter.sendMail(mailOptions);

  // Async OKKI sync (non-blocking, failures logged but don't affect email send)
  const uid = `smtp-${Date.now()}-${mailOptions.to.replace(/[^a-zA-Z0-9]/g, '').substring(0, 8)}`;
  syncEmailToOkki({
    uid,
    ...
    if (result.success) {
      console.error(`[OKKI Sync] Email synced to OKKI: trail_id=${result.trail_id}, customer=${result.customer?.name}`);
    } else {
      console.warn(`[OKKI Sync] Failed to sync email: ${result.reason} - ${result.message}`);
    }
  }).catch(err => {
    console.warn(`[OKKI Sync] Error: ${err.message}`);
  });
    ...
    switch (command) {
      case 'send':
        ...
        if (options['body-file']) {
          ...
          // Check if --html flag was provided
