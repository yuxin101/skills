...
if (_profile) {
  ...
  if (fs.existsSync(_pPath)) {
    ...
    console.log(`[imap] Using profile: ${_profile}`);
  } else {
    console.error(`[imap] Profile not found: ${_pPath}`);
    process.exit(1);
  }
}
...
// Search for messages (with optional maxResults to limit fetched messages)
function searchMessages(imap, criteria, fetchOptions, maxResults = 0) {
  return new Promise((resolve, reject) => {
    imap.search(criteria, (err, results) => {
      ...
      // If maxResults is set, only fetch the last N UIDs (newest by UID order)
      let fetchUids = results;
      if (maxResults > 0 && results.length > maxResults) {
        fetchUids = results.sort((a, b) => a - b).slice(-maxResults);
      }
...
async function checkEmails(mailbox = DEFAULT_MAILBOX, limit = 10, recentTime = null, unreadOnly = false) {
  ...
  try {
    ...
    // Only fetch headers for listing (much faster than full body)
    const fetchOptions = {
      bodies: ['HEADER.FIELDS (FROM TO SUBJECT DATE)'],
      markSeen: false,
    };

    const messages = await searchMessages(imap, searchCriteria, fetchOptions, limit);

    // Sort by UID descending (newest first)
    const sortedMessages = messages.sort((a, b) => {
      return (b.attributes.uid || 0) - (a.attributes.uid || 0);
    });
    ...
    for (const item of sortedMessages) {
      // Parse headers only
      const headerStr = item.body || '';
      const fromMatch = headerStr.match(/From:\s*(.+)/i);
      const toMatch = headerStr.match(/To:\s*(.+)/i);
      const subjectMatch = headerStr.match(/Subject:\s*(.+)/i);
      const dateMatch = headerStr.match(/Date:\s*(.+)/i);
    ...
    switch (command) {
      case 'check':
        result = await checkEmails(
          ...
          options.recent || '30d', // Default to last 30 days to avoid fetching all 1900+ emails
